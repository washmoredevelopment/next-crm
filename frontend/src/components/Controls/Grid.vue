  ...
  <div class="flex flex-col text-base">
    ...
    <div class="rounded border border-gray-100">
      ...
      >
        ...
        <div class="inline-flex items-center justify-center border-r py-2 px-1">No.</div>
        <div class="inline-flex items-center border-r p-2" v-for="field in gridFields" :key="field.fieldname">
          {{ field.label }}
        </div>
        <div class="p-2" />
      </div>
        ...
        <Draggable class="w-full" v-model="rows" group="rows" item-key="name">
          <template #item="{ element: row, index }">
            <div
              ...
              :style="{ gridTemplateColumns: gridTemplateColumns }"
            >
              <div class="inline-flex h-full items-center justify-center border-r p-2">
                <Checkbox
                  class="cursor-pointer duration-300"
              ...
              <div class="flex h-full items-center justify-center border-r p-2 text-sm text-ink-gray-8">
                {{ index + 1 }}
              </div>
              <div class="border-r border-gray-100 h-full" v-for="field in gridFields" :key="field.fieldname">
                <Link
                  ...
                  @change="(data: String) => field.onChange && field.onChange(data, index)"
                />
                <div v-else-if="field.fieldtype === 'Check'" class="flex h-full justify-center items-center">
                  <Checkbox
                    class="cursor-pointer duration-300"
                    v-model="row[field.fieldname]"
                    @change="
                      (e: Event) => field.onChange && field.onChange((e.target as HTMLInputElement).checked, index)
                    "
                  />
                </div>
                <FormControl
              ...
              <div class="edit-row">
                <Button class="flex w-full items-center justify-center rounded" @click="showRowList[index] = true">
                  <FeatherIcon name="edit-2" class="h-3.5 w-3.5 text-ink-gray-7" />
                </Button>
              </div>
              <Dialog v-model="showRowList[index]" :options="{ title: `Editing Row ${index + 1}` }">
    ...
    <div class="mt-2 flex flex-row gap-2">
      <Button v-if="showDeleteBtn" label="Delete" variant="solid" theme="red" @click="deleteRows" />
...
const gridTemplateColumns = computed(() => {
  ...
  columns += ' ' + props.gridFields.map((col) => `minmax(0, ${col.width || 2}fr)`).join(' ')
