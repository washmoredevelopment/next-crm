<template>
  <Popover transition="default" popoverClass="!z-[10000] pointer-events-auto">
    ...
    <template #body="{ togglePopover }">
      ...
      <div v-else class="my-3 max-w-max transform bg-surface-white px-4 sm:px-0" @pointerdown.stop>
        <div
        ...
        >
          <div class="flex gap-2 px-3 pb-1 pt-3">
            <div class="flex-1">
              <FormControl type="text" placeholder="Search by keyword" v-model="search" :debounce="300" />
            </div>
            <Button @click="setRandom">Random</Button>
          </div>
          <div class="w-96"></div>
          <div class="px-3" v-for="(emojis, group) in emojiGroups" :key="group">
            <div class="sticky top-0 bg-surface-modal pb-2 pt-3 text-sm text-ink-gray-7">
              {{ group }}
            </div>
...
const emojiGroups = computed(() => {
  ...
  for (let _emoji of gemoji) {
    if (search.value) {
      let keywords = [_emoji.description, ..._emoji.names, ..._emoji.tags].join(' ').toLowerCase()
