<template>
  <div>
        ...
        <ComboboxPortal>
          ...
          >
            <ComboboxViewport class="max-h-60 overflow-auto p-1.5">
              <ComboboxEmpty class="flex gap-2 rounded px-2 py-1 text-base text-ink-gray-5">
    ...
    <!-- Selected Attendees -->
    <div v-if="values.length" class="flex flex-col gap-2 mt-2 max-h-[165px] overflow-y-auto" ref="optionsRef">
      <Button
        ref="emails"
        v-for="att in values"
        :key="att.email"
      ...
      >
        ...
        <template #suffix>
          <FeatherIcon class="h-3.5" name="x" @click.stop="removeValue(att.email)" />
        </template>
      </Button>
...
const emptyStateText = computed(() =>
  props.fetchContacts ? __('No results found') : __('Type an email address to add attendee'),
)
