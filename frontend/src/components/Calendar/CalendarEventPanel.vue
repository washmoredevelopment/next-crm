  ...
  <div v-if="show" class="flex flex-col w-[352px] text-base">
    ...
    <div class="flex items-center justify-between p-4.5 text-ink-gray-7 text-lg font-medium">
      ...
      <div class="flex items-center gap-x-1">
        <ShortcutTooltip v-if="mode == 'details'" :label="__('Edit event')" combo="Enter">
        ...
        <ShortcutTooltip v-if="mode === 'edit' || mode === 'details'" :label="__('Duplicate event')" combo="Mod+D">
    ...
    <div v-if="mode == 'details'" class="flex flex-col overflow-y-auto">
      ...
      <div v-if="_event.referenceDocname" class="mx-4.5 my-2.5 border-t border-outline-gray-1" />
      <div v-if="_event.referenceDocname" class="flex items-center px-4.5 py-1 text-ink-gray-7">
        <component :is="_event.referenceDoctype == 'Lead' ? LeadsIcon : OpportunitiesIcon" class="size-4" />
        <Link
      ...
      <div v-if="peoples.length" class="mx-4.5 my-2.5 border-t border-outline-gray-1" />
      ...
      <div v-if="_event.description && _event.description !== '<p></p>'">
        ...
        <div class="px-4.5 py-2 text-ink-gray-7 text-p-base" v-html="_event.description" />
      </div>
    </div>
    ...
    <div v-else class="flex flex-col overflow-y-auto">
        ...
        <div class="flex items-center gap-x-1.5">
          ...
          >
            <template #suffix="{ togglePopover }">
              <FeatherIcon name="chevron-down" class="h-4 w-4 cursor-pointer" @click="togglePopover" />
            </template>
          </DatePicker>
        </div>
      </div>
      <div v-if="!_event.isFullDay" class="flex items-center justify-between px-4.5 py-[7px] text-ink-gray-7">
      ...
      <div v-if="_event.referenceDoctype" class="flex items-center justify-between px-4.5 py-[7px] text-ink-gray-7">
        <div class="">
          {{ _event.referenceDoctype == 'Lead' ? __('Lead') : __('Opportunity') }}
        </div>
        <div class="flex items-center gap-x-1.5">
          <Link
            class="w-[220px]"
            v-model="_event.referenceDocname"
            :doctype="_event.referenceDoctype"
            :filters="_event.referenceDoctype === 'Lead' ? { status: ['!=', 'Converted'] } : {}"
            variant="outline"
      ...
      <div class="flex items-center gap-2 px-4.5 py-[7px] text-ink-gray-7">
        <FormControl type="checkbox" v-model="_event.syncWithGoogleCalendar" @change="sync" />
        <label class="text-sm cursor-pointer" @click="_event.syncWithGoogleCalendar = !_event.syncWithGoogleCalendar">
          {{ __('Sync with Google Calendar') }}
        </label>
      </div>
      <div
        v-if="_event.syncWithGoogleCalendar"
        class="flex items-center justify-between px-4.5 py-[7px] text-ink-gray-7"
      >
        <div>{{ __('Google Calendar') }}</div>
        <Link
          class="w-[220px]"
          v-model="_event.googleCalendar"
    ...
    <div v-if="mode != 'details'" class="px-4.5 py-3">
      ...
      <div class="w-full">
        <Button
          ...
          class="w-full"
          :disabled="!dirty"
          :loading="mode === 'edit' ? events.setValue.loading : events.insert.loading"
          @click="saveEvent"
        >
          {{ mode === 'edit' ? __('Save') : mode === 'duplicate' ? __('Duplicate event') : __('Create event') }}
        </Button>
      </div>
    </div>
  </div>
...
const emit = defineEmits(['save', 'edit', 'delete', 'details', 'close', 'sync', 'duplicate'])
...
function fetchEvent(oldMode) {
  if (event.value.id && event.value.id !== 'new-event' && event.value.id !== 'duplicate-event') {
...
function showDiscardChangesModal(action) {
  $dialog({
    ...
    message: __('Are you sure you want to discard unsaved changes to this event?'),
    actions: [
      {
        label: __('Cancel'),
        onClick: (close) => {
  ...
  if (_event.value.referenceDocname) {
    ...
    let params =
      _event.value.referenceDoctype == 'Lead'
        ...
        : { opportunityId: _event.value.referenceDocname }
...
useKeyboardShortcuts({
  ...
  shortcuts: [
    ...
    {
      keys: 'Enter',
      guard: () => ['details', 'edit'].includes(props.mode) && props.mode === 'details',
      action: () => editDetails(),
    },
    {
