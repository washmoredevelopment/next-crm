    ...
    <template #body>
      <div class="rounded bg-surface-gray-7 py-1.5 px-2 text-xs text-ink-white shadow-xl">
        <span class="flex items-center gap-1">
          ...
          <template v-for="(combo, idx) in primaryCombosDisplay" :key="'prim-' + idx + combo">
            <KeyboardShortcut bg class="!bg-surface-gray-5 !text-ink-gray-2 px-1" :combo="combo" />
          </template>
          <!-- Alternate combos / equivalents -->
          <template v-for="(alt, idx) in altCombosDisplay" :key="'alt-' + idx + alt">
            <KeyboardShortcut bg class="!bg-surface-gray-5 !text-ink-gray-2 px-1" :combo="alt" />
          </template>
        </span>
      </div>
    </template>
...
function dedupeMacDeleteVariants(primary, alts) {
...
const deduped = computed(() => dedupeMacDeleteVariants(normalizedPrimary.value, normalizedAlt.value))
