...
// ToDos for calendar display (as all-day items on their due date)
const todos = createListResource({
  doctype: 'ToDo',
  cache: ['calendar-todos', user],
  fields: [
    'name',
    'status',
    'description',
  ...
  transform: (data) =>
    data
      .filter((todo) => todo.date) // Only show ToDos with a date
      .map((todo) => {
        // Extract first line of description as title (strip HTML)
        let title = __('ToDo')
        if (todo.description) {
        ...
        return {
          ...
          status: todo.status,
          fromDate: dayjs(todo.date).format('YYYY-MM-DD'),
          toDate: dayjs(todo.date).format('YYYY-MM-DD'),
          fromTime: '09:00',
          toTime: '10:00',
          isFullDay: true,
          eventType: 'Private',
          color: todo.priority === 'High' ? '#e74c3c' : todo.priority === 'Medium' ? '#f39c12' : '#3498db',
          referenceDoctype: todo.reference_type,
          referenceDocname: todo.reference_name,
          itemType: 'todo',
          todoName: todo.name,
        }
      }),
...
// Merged calendar items (events + todos)
const calendarItems = computed(() => {
  const eventData = events.data || []
  const todoData = todos.data || []
  return [...eventData, ...todoData]
})
...
function buildEventPayload(_event) {
  return {
    ...
    sync_with_google_calendar: _event.syncWithGoogleCalendar || false,
    google_calendar: _event.googleCalendar || null,
  }
}
